...
// Character side gallery: empty slots stay as tasteful archival placeholders;
// real images reveal themselves automatically when the matching file is uploaded.
document.querySelectorAll(".character-slot img").forEach(img => {
  const slot = img.closest(".character-slot");
  img.addEventListener("load", () => slot.classList.add("has-image"));
  img.addEventListener("error", () => slot.classList.remove("has-image"));
