...
## Character Side Gallery (10 spots)

There are 10 decorative character-image positions placed around the sides of the archive. They scroll naturally with the page, so each image stays at its own document position instead of following the screen.

To add your images, put these files beside `index.html`, `style.css`, and `script.js` in the GitHub repository:
...
You can use JPG, PNG, or WebP; if you use another extension, change the matching `src` in `index.html`. Missing images remain as subtle empty archival frames instead of broken-image icons. The positions are responsive and remain visible on desktop and mobile, just scaled down on smaller screens.
